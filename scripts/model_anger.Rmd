...
data_split <- initial_split(data_tfidf, prop = 0.7, strata = anger)
data_train <- training(data_split) %>%
  mutate(anger = factor(anger, levels = c("No", "Yes")))
...
manual_rf_recipe <- recipe(anger ~ ., data = model_data) %>%
...
for(i in 1:5) {
  ...
  manual_test_predictions <- predict(manual_final_fit, data_test, type = "prob") %>%
    mutate(anger = factor(data_test$anger, levels = c("No", "Yes")))
...
manual_res <- pROC::roc(manual_test_predictions$anger, manual_test_predictions$.pred_Yes)
...
manual_test_predictions_final <- manual_test_predictions %>%
  mutate(
    ...
    anger = factor(data_test$anger, levels = c("No", "Yes"))
  ) %>%
  select(pred, anger)

manual_confusion_matrix <- confusionMatrix(manual_test_predictions_final$pred, manual_test_predictions_final$anger)
...
grid_rf_recipe <- recipe(anger ~ ., data = model_data) %>%
  step_zv(all_predictors()) %>%
  step_normalize(all_predictors())
  # step_pca(all_predictors(), num_comp = 100)
...
grid_tune_results <- tune_grid(
  grid_rf_wf,
  resamples = vfold_cv(model_data, v = 5, strata = anger),
  grid = grid_rf_grid,
...
grid_res <- pROC::roc(grid_test_predictions$anger, grid_test_predictions$.pred_Yes)
...
grid_confusion_matrix <- confusionMatrix(grid_test_predictions_final$pred, grid_test_predictions_final$anger)
...
bayes_rf_recipe <- recipe(anger ~ ., data = model_data) %>%
  step_zv(all_predictors()) %>%
  step_normalize(all_predictors())
  # step_pca(all_predictors(), num_comp = 100)
...
bayes_res <- pROC::roc(bayes_test_predictions$anger, bayes_test_predictions$.pred_Yes)
...
bayes_confusion_matrix <- confusionMatrix(bayes_test_predictions_final$pred, bayes_test_predictions_final$anger)
...
for(i in 1:5) {
  ...
  predicted <- as.numeric(as.character(data_syuzhet$anger))
  actual <- as.numeric(as.character(data_gabarito$anger))
