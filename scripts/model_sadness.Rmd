...
data_split <- initial_split(data_tfidf, prop = 0.7, strata = sadness)
data_train <- training(data_split) %>%
  mutate(sadness = factor(sadness, levels = c("No", "Yes")))
...
manual_rf_recipe <- recipe(sadness ~ ., data = model_data) %>%
...
for(i in 1:5) {
  ...
  manual_test_predictions <- predict(manual_final_fit, data_test, type = "prob") %>%
    mutate(sadness = factor(data_test$sadness, levels = c("No", "Yes")))
...
manual_res <- pROC::roc(manual_test_predictions$sadness, manual_test_predictions$.pred_Yes)
...
manual_test_predictions_final <- manual_test_predictions %>%
  mutate(
    ...
    sadness = factor(data_test$sadness, levels = c("No", "Yes"))
  ) %>%
  select(pred, sadness)

manual_confusion_matrix <- confusionMatrix(manual_test_predictions_final$pred, manual_test_predictions_final$sadness)
...
grid_rf_recipe <- recipe(sadness ~ ., data = model_data) %>%
  step_zv(all_predictors()) %>%
  step_normalize(all_predictors())
  # step_pca(all_predictors(), num_comp = 100)
...
grid_tune_results <- tune_grid(
  grid_rf_wf,
  resamples = vfold_cv(model_data, v = 5, strata = sadness),
  grid = grid_rf_grid,
...
grid_res <- pROC::roc(grid_test_predictions$sadness, grid_test_predictions$.pred_Yes)
...
grid_confusion_matrix <- confusionMatrix(grid_test_predictions_final$pred, grid_test_predictions_final$sadness)
...
bayes_rf_recipe <- recipe(sadness ~ ., data = model_data) %>%
  step_zv(all_predictors()) %>%
  step_normalize(all_predictors())
  # step_pca(all_predictors(), num_comp = 100)
...
bayes_res <- pROC::roc(bayes_test_predictions$sadness, bayes_test_predictions$.pred_Yes)
...
bayes_confusion_matrix <- confusionMatrix(bayes_test_predictions_final$pred, bayes_test_predictions_final$sadness)
...
for(i in 1:5) {
  ...
  predicted <- as.numeric(as.character(data_syuzhet$sadness))
  actual <- as.numeric(as.character(data_gabarito$sadness))
