...
data_split <- initial_split(data_tfidf, prop = 0.7, strata = negative)
data_train <- training(data_split) %>%
  mutate(negative = factor(negative, levels = c("No", "Yes")))
...
manual_rf_recipe <- recipe(negative ~ ., data = model_data) %>%
...
for(i in 1:5) {
  ...
  manual_test_predictions <- predict(manual_final_fit, data_test, type = "prob") %>%
    mutate(negative = factor(data_test$negative, levels = c("No", "Yes")))
...
manual_res <- pROC::roc(manual_test_predictions$negative, manual_test_predictions$.pred_Yes)
...
manual_test_predictions_final <- manual_test_predictions %>%
  mutate(
    ...
    negative = factor(data_test$negative, levels = c("No", "Yes"))
  ) %>%
  select(pred, negative)

manual_confusion_matrix <- confusionMatrix(manual_test_predictions_final$pred, manual_test_predictions_final$negative)
...
grid_rf_recipe <- recipe(negative ~ ., data = model_data) %>%
  step_zv(all_predictors()) %>%
  step_normalize(all_predictors())
  # step_pca(all_predictors(), num_comp = 100)
...
grid_tune_results <- tune_grid(
  grid_rf_wf,
  resamples = vfold_cv(model_data, v = 5, strata = negative),
  grid = grid_rf_grid,
...
grid_res <- pROC::roc(grid_test_predictions$negative, grid_test_predictions$.pred_Yes)
...
grid_confusion_matrix <- confusionMatrix(grid_test_predictions_final$pred, grid_test_predictions_final$negative)
...
bayes_rf_recipe <- recipe(negative ~ ., data = model_data) %>%
  step_zv(all_predictors()) %>%
  step_normalize(all_predictors())
  # step_pca(all_predictors(), num_comp = 100)
...
bayes_res <- pROC::roc(bayes_test_predictions$negative, bayes_test_predictions$.pred_Yes)
...
bayes_confusion_matrix <- confusionMatrix(bayes_test_predictions_final$pred, bayes_test_predictions_final$negative)
...
for(i in 1:5) {
  ...
  predicted <- as.numeric(as.character(data_syuzhet$negative))
  actual <- as.numeric(as.character(data_gabarito$negative))
  ...
  syuzhet_specificity[i] <- conf_matrix$byClass['Specificity']
}

cat("A melhor acurácia foi de", max(syuzhet_accuracy), "com a limiar no valor de", which.max(syuzhet_accuracy)/5, "\n")
cat("Acurácia:", syuzhet_accuracy[which.max(syuzhet_accuracy)] %>% round(4), "\n")
cat("Sensibilidade:", syuzhet_sensibility[which.max(syuzhet_accuracy)] %>% round(4), "\n")
cat("Especificidade:", syuzhet_specificity[which.max(syuzhet_accuracy)] %>% round(4), "\n")
