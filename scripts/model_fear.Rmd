...
data_split <- initial_split(data_tfidf, prop = 0.7, strata = fear)
data_train <- training(data_split) %>%
  mutate(fear = factor(fear, levels = c("No", "Yes")))
...
manual_rf_recipe <- recipe(fear ~ ., data = model_data) %>%
...
for(i in 1:5) {
  ...
  manual_test_predictions <- predict(manual_final_fit, data_test, type = "prob") %>%
    mutate(fear = factor(data_test$fear, levels = c("No", "Yes")))
...
manual_res <- pROC::roc(manual_test_predictions$fear, manual_test_predictions$.pred_Yes)
...
manual_test_predictions_final <- manual_test_predictions %>%
  mutate(
    ...
    fear = factor(data_test$fear, levels = c("No", "Yes"))
  ) %>%
  select(pred, fear)

manual_confusion_matrix <- confusionMatrix(manual_test_predictions_final$pred, manual_test_predictions_final$fear)
...
grid_rf_recipe <- recipe(fear ~ ., data = model_data) %>%
  step_zv(all_predictors()) %>%
  step_normalize(all_predictors())
  # step_pca(all_predictors(), num_comp = 100)
...
grid_tune_results <- tune_grid(
  grid_rf_wf,
  resamples = vfold_cv(model_data, v = 5, strata = fear),
  grid = grid_rf_grid,
...
grid_res <- pROC::roc(grid_test_predictions$fear, grid_test_predictions$.pred_Yes)
...
grid_confusion_matrix <- confusionMatrix(grid_test_predictions_final$pred, grid_test_predictions_final$fear)
...
bayes_rf_recipe <- recipe(fear ~ ., data = model_data) %>%
  step_zv(all_predictors()) %>%
  step_normalize(all_predictors())
  # step_pca(all_predictors(), num_comp = 100)
...
bayes_res <- pROC::roc(bayes_test_predictions$fear, bayes_test_predictions$.pred_Yes)
...
bayes_confusion_matrix <- confusionMatrix(bayes_test_predictions_final$pred, bayes_test_predictions_final$fear)
...
for(i in 1:5) {
  ...
  predicted <- as.numeric(as.character(data_syuzhet$fear))
  actual <- as.numeric(as.character(data_gabarito$fear))
