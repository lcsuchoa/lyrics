...
data_split <- initial_split(data_tfidf, prop = 0.7, strata = positive)
data_train <- training(data_split) %>%
  mutate(positive = factor(positive, levels = c("No", "Yes")))
...
manual_rf_recipe <- recipe(positive ~ ., data = model_data) %>%
...
for(i in 1:5) {
  ...
  manual_test_predictions <- predict(manual_final_fit, data_test, type = "prob") %>%
    mutate(positive = factor(data_test$positive, levels = c("No", "Yes")))
...
manual_res <- pROC::roc(manual_test_predictions$positive, manual_test_predictions$.pred_Yes)
...
manual_test_predictions_final <- manual_test_predictions %>%
  mutate(
    ...
    positive = factor(data_test$positive, levels = c("No", "Yes"))
  ) %>%
  select(pred, positive)

manual_confusion_matrix <- confusionMatrix(manual_test_predictions_final$pred, manual_test_predictions_final$positive)
...
grid_rf_recipe <- recipe(positive ~ ., data = model_data) %>%
  step_zv(all_predictors()) %>%
  step_normalize(all_predictors())
  # step_pca(all_predictors(), num_comp = 100)
...
grid_tune_results <- tune_grid(
  grid_rf_wf,
  resamples = vfold_cv(model_data, v = 5, strata = positive),
  grid = grid_rf_grid,
...
grid_res <- pROC::roc(grid_test_predictions$positive, grid_test_predictions$.pred_Yes)
...
grid_confusion_matrix <- confusionMatrix(grid_test_predictions_final$pred, grid_test_predictions_final$positive)
...
bayes_rf_recipe <- recipe(positive ~ ., data = model_data) %>%
  step_zv(all_predictors()) %>%
  step_normalize(all_predictors())
  # step_pca(all_predictors(), num_comp = 100)
...
bayes_res <- pROC::roc(bayes_test_predictions$positive, bayes_test_predictions$.pred_Yes)
...
bayes_confusion_matrix <- confusionMatrix(bayes_test_predictions_final$pred, bayes_test_predictions_final$positive)
