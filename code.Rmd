...
```{r}
genres <- c("bossa-nova", "forro", "funk-carioca", "gospel", "mpb")

all_artists_link <- future_map(genres, ~get_artist_link(.)) %>%
  unlist()
```
...
tic()
all_songs <- future_map_dfr(all_artists_link, ~scrap_lyrics_link(.)) %>%
  filter(!is.na(Slink))
toc()
...
```{r}
all_songs <- read.csv("src/all_songs.csv")
data_songs <- read.csv("src/data_songs.csv")
```
